...
import { AddWorkItemQueueResponse, DeleteWorkItemQueueResponse, DeleteWorkitemResponse, PopWorkitemResponse, PushWorkitemResponse, UpdateWorkItemQueueResponse, UpdateWorkitemResponse } from "./proto/workitems.js";
...
export class protowrap {
            ...
            try {
                if (message != null) {
                    if (!this.IsPendingReply(result, message)) {
                        ...
                        if (_payload && _payload.command != "noop") this.sendMesssag(result, _payload, undefined, true);
                    }
                }
            } catch (error) {
                err(error);
        ...
        if (typeof data == "string") {
            ...
            switch (command) {
                ...
                case "addworkitemqueuereply":
                    msg = AddWorkItemQueueResponse.decode(data);
                    break;
    ...
    static RPC(client: client, payload: Envelope) {
    ...
    static SetStream(client: client, stream:any, rid: string) {
        client.streams[rid] = { stream, chunks: 0, bytes: 0 };
        return client.streams[rid];
    }
    static DownloadFile(client: client, id:string, collectionname:string, filename:string, jwt: string) {
        return new Promise<any>(async (resolve, reject) => {
            try {
                ...
                const payload: Envelope = Envelope.create({ command: "download", data, jwt: jwt });
    ...
    static UploadFile(client: client, filename:string, mimetype:string, content:Uint8Array, jwt: string): Promise<string> {
        return new Promise<any>(async (resolve, reject) => {
            const uploaddata = Any.create<any>({"typeUrl": "type.googleapis.com/openiap.UploadRequest", 
            "value": UploadRequest.encode(UploadRequest.create({filename, mimetype})).finish()})
            const payload: Envelope = Envelope.create({ command: "upload", data: uploaddata, jwt: jwt });
            const [rid, promise] = this._RPC(client, payload);
    
            const chunksize = 5 * 1024 * 1024;
            const chunks = Math.ceil(content.length/chunksize)
            var counter = 0;
            const beginstreamdata = Any.create<any>({"typeUrl": "openiap.beginstream", "value": BeginStream.encode(BeginStream.create()).finish()})
            ...
            for(let start = 0; start < content.length; start = start + chunksize) {
                ...
                const chunkdata = Any.create<any>({"typeUrl": "openiap.stream", "value": Stream.encode(Stream.create({ data })).finish()})
                this.sendMesssag(client, Envelope.create({ rid, 
                    command: "stream", data: chunkdata}), undefined, true);
            }
            const endstreamdata = Any.create<any>({"typeUrl": "openiap.endstream", "value": EndStream.encode(EndStream.create()).finish()})
            this.sendMesssag(client, Envelope.create({ rid, 
